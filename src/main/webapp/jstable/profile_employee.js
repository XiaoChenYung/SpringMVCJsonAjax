...
function editFormModal(
    ...
    images.replaceWith(images = images.clone(true));
    $('#account-modal-title').html('Edit Employee');
    $('#uploading').hide();
...
function setReadOnlyFieldsOnForm(isEnable) {
    $('#frmModal #eid').attr("disabled", isEnable);
    ...
    $('#frmModal #employeeNo').attr("disabled", isEnable);
...
function showFormModal(
    ...
    console.log('showFormModal is called with eid=' + eid);
    if (eid == null || eid == "") {
        return;
    }
    mobileUnum = mobile;
    //var modal = $('#editAccountModal');
    $('#account-modal-title').html('View employee');

    $('#frmModal #actionType').val('ACTION_NONE');

    $('#frmModal #eid').val(eid);
    ...
    $('#frmModal #employeeNo').val(employeeNo);
...
function updateStatusDeleteRestore(eid, isDirty) {
    console.log('updateStatusDeleteRestore is called with eid=' + eid);
    if (eid == null || eid == "") {
        return;
    }
    var dataString = 'eid=' + eid + '&isDirty= ' + isDirty;
    $.ajax({
        type: "POST",
...
checkEditAndSubmitForm = function ()
{
    ...
    var pass = $("#pass").val();
    var url_submit = '';
    url_submit = './api/employee/update/';
...
function getProfileEmployee() {
    ...
    $.ajax({
        type: "GET",
        url: "./api/profile_employee/getProfile/1?userId=" + userId,
        cache: false,
        dataType: "json",
        beforeSend: function () {
            //$("#submit").val('Connecting...');
        ...
        success: function (data) {
            if (data.error_code == 0) {
                ...
                $('#frmModal #eid').val(employee.eid);
                $('#frmModal #userId').val(employee.userId);
                $('#frmModal #login').val(employee.login);
                $('#frmModal #pass').val(employee.password);
                $('#frmModal #password').val('');
                $('#frmModal #oid').val(employee.oid);

                $('#frmModal #organisation').val(employee.organisation);
                $('#frmModal #name').val(employee.name);
                $('#frmModal #mobile').val(employee.mobile);
                $('#frmModal #email').val(employee.email);
                $('#frmModal #address').val(employee.address);// decode address with special characters
                $('#frmModal #dob').val(employee.dob);
                $('#frmModal #designation').val(employee.designation);
                $('#frmModal #department').val(employee.department);
                $('#frmModal #standard').val(employee.standard);
                $('#frmModal #section').val(employee.section);
                $('#frmModal #blood').val(employee.blood);
                $('#frmModal #employeeNo').val(employee.employeeNo);
                $('#frmModal #rfidNo').val(employee.rfidNo);
                currentShift = employee.shiftsch;
                getOrganisationById();
                $('#frmModal #shiftsch').val(employee.shiftsch);
                if (employee.alertType == null || employee.alertType == '') {
                    $("#frmModal #alertTypeSMS").prop("checked", false);
                    $("#frmModal #alertTypeEmail").prop("checked", false);
                    $("#frmModal #alertTypeNotification").prop("checked", false);
                } else {
                    $("#frmModal #alertTypeSMS").prop("checked", employee.alertType.SMS);
                    $("#frmModal #alertTypeEmail").prop("checked", employee.alertType.Email);
                    $("#frmModal #alertTypeNotification").prop("checked", employee.alertType.Notification);
                }
                $('#frmModal #gender').val(employee.gender);
                $('#frmModal #religion').val(employee.religion);
                $('#frmModal #qualification').val(employee.qualification);
                $('#frmModal #martial').val(employee.martial);
                $('#frmModal #experience').val(employee.experience);
                $('#frmModal #medical').val(employee.medical);
                $('#frmModal #reference').val(employee.reference);
                $('#frmModal #court').val(employee.court);
                $('#frmModal #valid').val(employee.valid);

                if (employee.picture != '') {
                    $('#frmModal #prviewLogoDiv').show();
                    $('#frmModal #prviewLogo').attr('src', 'uploads/' + employee.picture);
                    $('.logoHeader').attr('src', 'uploads/' + employee.picture);
                    $('#logoLeft').attr('src', 'uploads/' + employee.picture);
                } else {
                    $('#frmModal #prviewLogo').attr('src', '');
                }
                $('.organisation').text(employee.name);
                $("#loginDiv").text(employee.login);
                if (employee.picture != '') {
                    $('.logoHeader').attr('src', 'uploads/' + employee.picture);
                    $('#logoLeft').attr('src', 'uploads/' + employee.picture);
                }
